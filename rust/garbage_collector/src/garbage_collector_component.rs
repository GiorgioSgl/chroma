...
/// Maximum number of attached functions to garbage collect per run
const MAX_ATTACHED_FUNCTIONS_TO_GC_PER_RUN: i32 = 100;
...
impl GarbageCollector {
    ...
    async fn prune_heap_across_shards(&self, cutoff_time: chrono::DateTime<chrono::Utc>) {
        ...
        loop {
            ...
            // Check if heap exists by creating a reader first
            // HeapReader::new checks for initialization but HeapPruner::new doesn't
            let heap_exists = match s3heap::HeapReader::new(
                self.storage.clone(),
                heap_prefix.clone(),
                Arc::clone(&scheduler),
            )
            .await
            {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Error checking heap existence for shard {}: {:?} - continuing",
                        service_index,
                        e
                    );
                    service_index += 1;
            ...
            // Create HeapPruner for this shard
            let pruner = match s3heap::HeapPruner::new(
                ...
                Err(e) => {
                    tracing::error!(
                        "Failed to create HeapPruner for shard {}: {:?}",
                        service_index,
                        e
                    );
                    service_index += 1;
